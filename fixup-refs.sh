...
IMAGE_NUMBERS=$(grep -E "${DOC_BASENAME}([0-9]+)\.(png|gif)" "${DOC}" | sed -Ee "s/.*${DOC_BASENAME}([0-9]+)\.(png|gif).*/\1/g" | tr '\n' ' ')
...
do
  ...
  sed -Ee "s/${DOC_BASENAME}${I}\.(png|gif)/${DOC_BASENAME}\.image-${NEW_I}\.svg/g" "${DOC}" > "${DOC}.tmp"
  mv "${DOC}.tmp" "${DOC}"
